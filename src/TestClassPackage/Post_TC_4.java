...
public class Post_TC_4 {
	...
	public static void execute() throws IOException
	{
		...
		int statusCode = Post_Common_Function.statusCode(post_req_repository.baseURI(),post_req_repository.resource(),post_req_repository.requestBody4());
		if(statusCode==201) {
		System.out.println("status code is " +statusCode);
		
		
		String responseBody =  Post_Common_Function.responseBody(post_req_repository.baseURI(),post_req_repository.resource(),post_req_repository.requestBody4());
		Post_TC_4.validator(responseBody, statusCode);
		Utility_Common_Function.evidencefilecreater("Ashu",post_req_repository.requestBody4() , responseBody);
	...
	System.out.println(post_req_repository.requestBody4());
	
		//parse the request and response body
	
		JsonPath jspr = new JsonPath(post_req_repository.requestBody4());
